...
void draw() {
  int outerCircleRadius = 15 - frameCount % 16;
  for(int i = 0; i < BPL_NUM; i++)
    bpl[i].zeros();
  int mod = frameCount % 16;
  int k = 4;
  if(mod < 12)
    k = 3;
  if(mod < 8)
  ...
  if(outerCircleRadius > 0) {
    circles[(circ > 0) ? circ-1 : CIRC_COUNT -1].drawOuterCircle((16-outerCircleRadius)/2);
    circles[circ].drawOuterCircle(outerCircleRadius/2);
    mirror[(circ > 0) ? circ-1 : CIRC_COUNT -1].drawOuterCircle((16-outerCircleRadius)/2);
